...
def test_random_orders():
    ...
    for i in range(10_000):
        ob.insert_order(Order(order_id=i, side=Side.B, size=random.randint(1, 500),
                              price=random.randint(1, 5000), exchange=ob.exchange,
                              lifespan=Lifespan.F, status=OrderStatus.C))
    print("Done adding orders, starting fills")
    for i in range(10_000):
        ob.insert_order(Order(order_id=i+10000, side=Side.A, size=random.randint(1, 500),
                              price=random.randint(1, 5000), exchange=ob.exchange,
                              lifespan=Lifespan.F, status=OrderStatus.C))
    print(ob.ask_book.prices)
    print(ob.ask_book.levels)
    print(ob.ask_book.total_volumes)
    
    print("\n")
    print(ob.bid_book.prices)
    print(ob.bid_book.levels)
    print(ob.bid_book.total_volumes)
